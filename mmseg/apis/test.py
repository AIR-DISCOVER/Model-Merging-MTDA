...
from mmseg.models.uda import DTDA
...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            if isinstance(model.module, DTDA):
                result = model(return_loss=False, decoder_norm_select=decoder_norm_select, **data)
            else:
                result = model(return_loss=False, **data)
        # result[0][result[0] == 9] = 0
        # result[0][result[0] == 1] = 0
        # result[0][result[0] == 2] = 3
        # breakpoint()
        # if show or out_dir:
