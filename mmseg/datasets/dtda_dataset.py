...
class DTDADataset(object):  # developer version: Dual-Target Domain Adaptation

    def __init__(self, source, target1, target2, cfg):
        self.source = source
        self.target1 = target1
        self.target2 = target2
        self.ignore_index = target1.ignore_index
        self.CLASSES = target1.CLASSES
        self.PALETTE = target1.PALETTE
        assert target1.ignore_index == source.ignore_index
        assert target1.CLASSES == source.CLASSES
        assert target1.PALETTE == source.PALETTE
    ...
    # added support for second target dataset
    ...
    def get_rare_class_sample(self):
        ...
        if self.rcs_min_crop_ratio > 0:
            for j in range(10):
                ...
                s1 = self.source[i1]
        i2 = np.random.choice(range(len(self.target1)))
        s2 = self.target1[i2]

        # sample second target dataset
        ...
        out = {
            **s1, 
            'target1_img_metas': s2['img_metas'],
            'target1_img': s2['img'],
            'target2_img_metas': s3['img_metas'],
            'target2_img': s3['img']
        }
        if 'valid_pseudo_mask' in s2:
            out['target1_valid_pseudo_mask'] = s2['valid_pseudo_mask']
        if 'valid_pseudo_mask' in s3:
            out['target2_valid_pseudo_mask'] = s3['valid_pseudo_mask']
    ...
    def __getitem__(self, idx):
        if self.rcs_enabled:
            ...
            # s2 = self.target1[idx % len(self.target)]
    ...
    def __len__(self):
        return int(len(self.source) * len(self.target1) * 1.5)
        # return len(self.source) * len(self.target)
        # return len(self.source) * len(self.target1) * len(self.target2)  # not sure on this
