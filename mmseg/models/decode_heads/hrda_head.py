...
class HRDAHead(BaseDecodeHead):
        ...
        if single_scale_head == 'DAFormerHead':
            ...
            if attn_cfg['decoder_params']['fusion_cfg']['type'] == 'aspp':
                attn_cfg['decoder_params']['fusion_cfg'] = dict(
                    ...
                    # dup_norm=attn_cfg['decoder_params']['fusion_cfg']['dup_norm'])
    ...
    def decode_hr(self, inp, bs, norm_select=0):
        if isinstance(inp, dict) and 'boxes' in inp.keys():
            ...
            crop_seg_logits = self.head(features, norm_select=norm_select)
            for i in range(len(boxes)):
                y1, y2, x1, x2 = boxes[i]
            ...
            return self.head(inp, norm_select=norm_select)

    def get_scale_attention(self, inp, norm_select=0):
        if self.scale_attention is not None:
            att = torch.sigmoid(self.scale_attention(inp, norm_select=norm_select))
        else:
            att = self.fixed_attention
        return att

    def forward(self, inputs, norm_select=0):
        assert len(inputs) == 2
        hr_inp = inputs[1]
        ...
        lr_seg = self.head(lr_inp, norm_select=norm_select)
        # print_log(f'lr_seg {lr_seg.shape}', 'mmseg')

        hr_seg = self.decode_hr(hr_inp, batch_size, norm_select=norm_select)

        att = self.get_scale_attention(lr_sc_att_inp, norm_select=norm_select)
        if has_crop:
    ...
    def forward_train(self,
        ...
        seg_logits = self.forward(inputs, norm_select=norm_select)
        losses = self.losses(seg_logits, gt_semantic_seg, seg_weight)
        self.reset_crop()
        return losses

    def forward_test(self, inputs, img_metas, test_cfg, norm_select=0):
        """Forward function for testing, only ``fused_seg`` is used."""
        return self.forward(inputs, norm_select=norm_select)[0]
