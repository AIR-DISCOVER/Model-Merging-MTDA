...
from mmseg.models.bricks import ConvModule  # custom mmcv.cnn.bricks.conv_module
...
class ASPPWrapper(nn.Module):
    ...
    def forward(self, x, norm_select=0):
        ...
        aspp_outs.extend(self.aspp_modules(x, norm_select=norm_select))
        aspp_outs = torch.cat(aspp_outs, dim=1)

        output = self.bottleneck(aspp_outs, norm_select=norm_select)
        return output
...
class DAFormerHead(BaseDecodeHead):
    ...
    def forward(self, inputs, norm_select=0):
        ...
        x = self.fuse_layer(torch.cat(list(_c.values()), dim=1), norm_select=norm_select)
        # if isinstance(self.fuse_layer, ConvModule):
        #     print("decoder scale attention fuse layer using norm id -> ", id(getattr(self.fuse_layer, f"bn{norm_select}")))
        x = self.cls_seg(x)
